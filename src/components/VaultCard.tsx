...
export function VaultCard({ item, onRemove }: VaultCardProps) {
  ...
  const accentColor = isNews ? "blue" : "orange";
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 flex-wrap">
            <span className={`font-medium ${isNews ? "text-blue-600 dark:text-blue-400" : "text-orange-600 dark:text-orange-400"}`}>
              {item.source}
            </span>
            <span>·</span>
        ...
        <div className="flex-shrink-0 self-center flex items-center gap-2">
          <button
            ...
            className="p-1.5 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 opacity-0 group-hover:opacity-100 transition-all"
